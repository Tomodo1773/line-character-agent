...
Usage:
    python download_secrets_from_keyvault.py --keyvault-name <vault_name> [options]

Example:
    # Download all secrets to .env file
    python download_secrets_from_keyvault.py --keyvault-name my-keyvault --output .env
    
    # Download only project secrets to stdout
    python download_secrets_from_keyvault.py --keyvault-name my-keyvault --project-only
    
    # Show all available secrets (dry run)
    python download_secrets_from_keyvault.py --keyvault-name my-keyvault --list-only
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Download secrets from Azure Key Vault and output in .env format"
    )
    parser.add_argument(
        "--keyvault-name", 
        required=True, 
        help="Azure Key Vault name (without .vault.azure.net suffix)"
    )
    parser.add_argument(
        "--output", 
        "-o",
    ...
    parser.add_argument(
        "--project-only", 
        action="store_true", 
        help="Only download secrets used by this project"
    )
    parser.add_argument(
        "--list-only", 
        action="store_true", 
        help="Only list available secrets without downloading values"
    )
