...
def test_chatbot_agent_response():
    ...
    with patch("chatbot.agent.character_graph.nodes.get_user_profile", return_value=""):
        with patch("chatbot.agent.character_graph.nodes.get_user_digest", return_value=""):
            # OAuth設定がないテスト環境では ensure_oauth_node と ensure_drive_folder_node をスキップ
            with patch(
                "chatbot.agent.character_graph.graph.ensure_oauth_node",
                return_value=Command(goto="ensure_drive_folder"),
            ):
                with patch(
                    "chatbot.agent.character_graph.graph.ensure_drive_folder_node",
...
def test_ensure_oauth_node_returns_auth_interrupt(monkeypatch):
...
def test_ensure_drive_folder_node_registers_folder_id(monkeypatch):
    ...
    result = ensure_drive_folder_node(state)
