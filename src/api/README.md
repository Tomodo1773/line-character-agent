...
1. `.env` に `GOOGLE_CLIENT_ID`、`GOOGLE_CLIENT_SECRET`、`GOOGLE_OAUTH_REDIRECT_URI` を設定する（フォルダIDはこれまで通り `DRIVE_FOLDER_ID` で管理）。
...
※ トークン保存は `GOOGLE_TOKEN_ENC_KEY`（Fernet鍵、Key Vault経由の環境変数）で暗号化してから `users` コンテナへ保存します。フォルダIDのユーザー登録は次期対応として環境変数を継続使用しています。
